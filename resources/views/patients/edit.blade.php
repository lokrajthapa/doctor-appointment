...
<form action="{{ route('patients.update', $patient->id) }}" method="POST" class="space-y-6 m-4">
    @csrf
    @method('PUT')
    ...
    <div class="form-group">
        ...
        <input type="hidden" name="user_id" id="user_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" value="{{ $patient->user_id }}" required>
    </div>
